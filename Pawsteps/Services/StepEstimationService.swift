...
class StepEstimationService: StepEstimationServiceProtocol, ObservableObject {
    ...
    func calculateDailyGoal(for breedName: String, bodyCondition: DogBodyCondition) -> Int {
        ...
        // Calculate goal based on breed-specific physical characteristics
        let baseGoal = calculateBreedSpecificGoal(breedInfo)
        ...
        // Adjust for energy level from breed data
        let energyMultiplier: Double
        switch breedInfo.movement.energyLevel {
    ...
    private func calculateBreedSpecificGoal(_ breedInfo: BreedInfo) -> Int {
        // Use breed-specific physical characteristics for more accurate goal calculation
        let legLength = breedInfo.physical.averageLegLengthCm
        let weight = breedInfo.physical.averageWeightKg
        let bodyType = breedInfo.physical.bodyType
        
        // Base calculation using leg length and weight
        // Longer legs = longer strides = fewer steps needed
        // Heavier dogs = more effort per step = fewer steps needed
        let legLengthFactor = max(0.5, min(2.0, 25.0 / legLength)) // Normalized to 0.5-2.0 range
        let weightFactor = max(0.7, min(1.5, 25.0 / weight)) // Normalized to 0.7-1.5 range
        
        // Body type adjustments
        let bodyTypeMultiplier: Double
        switch bodyType {
        case .compact: bodyTypeMultiplier = 1.2  // Shorter strides, more steps
        case .athletic: bodyTypeMultiplier = 1.0  // Efficient movement
        case .elongated: bodyTypeMultiplier = 0.9  // Longer strides, fewer steps
        case .heavy: bodyTypeMultiplier = 0.8  // Deliberate movement, fewer steps
        }
        
        // Calculate base daily goal
        let baseGoal = Int(8000 * legLengthFactor * weightFactor * bodyTypeMultiplier)
        
        // Clamp to reasonable range
        let clampedGoal = max(2000, min(15000, baseGoal))
        
        print("🐕 [StepEstimationService] Breed-specific goal calculation for \(breedInfo.name): leg=\(legLength)cm, weight=\(weight)kg, body=\(bodyType.rawValue) → \(clampedGoal) steps")
